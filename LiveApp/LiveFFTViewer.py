...
amplitudes = .00001 + .00002 * np.random.rand(m, 1).astype(np.float32)
...
class Canvas(app.Canvas):
    def __init__(self):
        ...
        self._timer = app.Timer(0.1, connect=self.on_timer, start=True)
    ...
    def on_timer(self, event):
        ...
        k = 4410
        y[:, :-k] = y[:, k:]
        y[:, -k:] = amplitudes * mc.data.reshape((1,k)).astype(np.float32)
